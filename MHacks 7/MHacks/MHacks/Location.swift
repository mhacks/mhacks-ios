...
@objc final class Location : NSObject {
	...
	convenience init?(serialized: Serialized) {
		...
		guard let latitude = serialized.doubleValueForKey(Location.latitudeKey), let longitude = serialized.doubleValueForKey(Location.longitudeKey), let id = serialized[Location.idKey] as? String, let locationName = serialized[Location.nameKey] as? String
			else
		{
			return nil
		}
...
extension Location : JSONCreateable {
	...
	@objc func encodeWithCoder(aCoder: NSCoder) {
		...
		aCoder.encodeDouble(coreLocation.coordinate.latitude, forKey: Location.latitudeKey)
